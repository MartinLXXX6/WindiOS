...
:HandleMainMenuInput
	IF /I "%userInput%" EQU "T" (
		...
		%WINDIR%\System32\REG.EXE QUERY "HKCU\Software\Microsoft\MobilePC\MobilityCenter" >NUL 2>&1
		IF ERRORLEVEL 1 (
...
:HandleToolsMenuInput
	IF /I "%userInput%" EQU "0" (
		...
		CALL :RunPowershellCommand "%WINDIR%\System32\WindowsPowershell\v1.0\POWERSHELL.EXE"
...
:HandlePowerMenuInput
	IF /I "%userInput%" EQU "B" (
		...
		ECHO NOTE: IF NOTHING HAPPENS THAT MEANS THAT YOUR FIRMWARE DOES NOT SUPPORT IT.
		SET /P "confirmation=ARE YOU SURE YOU WANT TO REBOOT TO UEFI? (Y)(N): "
		IF /I "!confirmation!" EQU "Y" (
			rem CALL :RunPowershellCommand "Start-Process '%WINDIR%\System32\CMD.EXE' -ArgumentList '/C %WINDIR%\System32\SHUTDOWN.EXE /FW /R /T 0 > \"%TEMP%\WindiOS_temp.txt\" 2>&1' -Verb RunAs"
			CALL :RunPowershellCommand "Start-Process '%WINDIR%\System32\SHUTDOWN.EXE' -ArgumentList '/FW /R /T 0' -Verb RunAs"
		)
	) ELSE IF /I "%userInput%" EQU "5" (
...
:RunPowershellCommand
	START "" "%WINDIR%\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -Command "%~1"
	EXIT /B
